...
ユーザーが指定した静的モック制作指示をもとに、ClaudeCode が Codex 実装用プロンプトを作成し、Codex に実装を委任し、実装結果をレビューする Command です。

ユーザーは以下の形式で実行します。

```
/codex-exec [静的モック制作に関する具体指示]
```
...
### Step 1: ユーザー指示を確認する
`$ARGUMENTS` を今回の制作指示として読む。
- 作るべき画面・目的・顧客確認ポイント・仮仕様・デザイン希望を把握する
- 不足情報があればユーザーに確認する（推測で補完しない）

### Step 2: 短い Plan をユーザーへ提示する
実装を開始する前に以下を提示し、方針のズレがないか確認する。
- 実装する画面・ファイル
- Edit scope（変更するファイル・ディレクトリ）
- 主な実装内容の概要
