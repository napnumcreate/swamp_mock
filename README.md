...
**Agent 基盤 ＋ HTML/CSS テンプレート基盤が整備済み**です。実モック画面は案件ごとに `/codex-exec` で追加します。
...
| パス | 内容 |
|---|---|
| `docs/index.html` | 公開トップのプレースホルダー |
...
Template Repository として複製した後は、以下の流れで使います。
...
6. **`docs/index.html` に生成ページへの導線（リンク）を追加する**（複数ページの場合）
7. **GitHub Pages の URL を顧客へ共有する**

### 顧客共有導線について

- 複数ページのモックを作る場合は、**原則として `docs/index.html` から主要ページへ遷移できる形**にする
- ルート URL（`https://<org>.github.io/<repo>/`）を顧客へ共有する場合は、`docs/index.html` の案内文・リンク導線を確認してから共有する
